...
//Set the matrix type to the type commonly found on eBay. Read MD_Parola documentation for which option to use for other modules
...
const int BUF_SIZE = 255;           //Don't change this unless you also change it in the index.h file. At some point it would be good to fix this...
...
void setup() {
  ...
  incomingFS.toCharArray(newMessage, 255);
...
void handleRoot() {
  server.send(200, "text/html", mainPage);    //Send mainPage array as HTML
}
...
void handleForm() {
  ...
  incomingMessage.toCharArray(newMessage, 255);             //Convert incoming message to a char array (much better);
